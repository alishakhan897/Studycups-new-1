...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onSubmit }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                // यदि स्टेटस 400 या 500 है (एरर)
                
                // डुप्लीकेट नंबर एरर (400) हैंडलिंग
                if (res.status === 400 && data.message.includes("registered")) {
                    // Mobile Number फ़ील्ड पर एरर दिखाएँ
                    setErrors({ mobileNumber: data.message });
                } else {
                    // अन्य एरर के लिए (जैसे सर्वर एरर)
                    alert(`Submission Error: ${data.message || "Something went wrong."}`);
                }
                
                setLoading(false);
                return; // एरर होने पर यहाँ रुक जाएँ, onSubmit को कॉल न करें
            }

            // 2. सबमिशन सफल होने पर (res.ok is true, Status 201)
            
            // send name to parent → pass page open
            onSubmit(formData.studentName.trim(), formData.event);

            // सफलता पर लोडिंग बंद करें
            setLoading(false);

        } catch (err) {
            // नेटवर्क या अन्य अनपेक्षित एरर
